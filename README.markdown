#Float port of SVGo with different precisions
```
This is a port of SVGo to support floating point coordinates. 
The library was automatically generated by modifying the syntax tree. 
(No human beings were harmed, only the ast package.)

There are different precisions available:
- svgof: formatted with "%f"
- svgo1f: formatted with "%.1f"
- svgo2: formatted with "%.2f"
- ...

Right now only float64 is available, but float32 is possible if there is any demand. 
```
Use at your own risk, the import path might change in the future.  Right now for double precision it is for example:
```go
go get "https://github.com/stanim/svgo/tree/master/svgo2f"
```
The package is also called `svgo`, so you don't need to rewrite your existing code.

*Please only file an issue if something works with [github.com/ajstarks/svgo](https://github.com/ajstarks/svgo), but not with this port. Otherwise report it [upstream](https://github.com/ajstarks/svgo).*
