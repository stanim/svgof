...
func (p *Parameters) barchart(location string, canvas *svg.SVG) {
	...
	canvas.Gstyle(fmt.Sprintf("stroke-opacity:%.2f;stroke:%s;stroke-width:%.9f;text-anchor:middle", p.opacity, p.barcolor, bw))
...
func main() {
	...
	canvas.Gstyle(fmt.Sprintf("font-family:Calibri;font-size:%.9fpx", param.fontsize))
	if len(flag.Args()) == 0 {
