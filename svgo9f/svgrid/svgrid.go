...
func placepic(x, y float64, filename string) (float64, float64) {
	...
	nw, _ := fmt.Sscanf(s.Width, "%.9f%s", &width, &wunit)
	if nw < 1 {
		width = 10.0
	}
	nh, _ := fmt.Sscanf(s.Height, "%.9f%s", &height, &hunit)
	if nh < 1 {
		height = 10.0
	}
	canvas.Group(`clip-path="url(#pic)"`, fmt.Sprintf(`transform="translate(%.9f,%.9f)"`, x, y))
