...
func defrotate(id string, w, h float64, deg float64, legend string) {
	...
	deflegend(w/2, 0, h, legend)
	canvas.Rect(0, 0, w, h, objstyle)
	canvas.Qbez(w/2, 0, (w/2)+10, float64(ry)/2, float64(rx), float64(ry), "fill:none;stroke:gray")
...
func defrgb(id string, w, h, r, g, b float64, opacity float64, legend string) {
	size := h / 8.0
	canvas.Gid(id)
...
func colordot(x, y, r, red, green, blue float64, a float64) {
	// canvas.Circle(x,y,r+textsize/6,"fill:none;stroke:"+objcolor)
	if a == 1.0 {
		canvas.Circle(x, y, r, canvas.RGB(int(red), int(green), int(blue)))
	} else {
		canvas.Circle(x, y, r, canvas.RGBA(int(red), int(green), int(blue), a))
	}
}
