...
func process(canvas *svg.SVG, filename string, g geometry) float64 {
	if filename == "" {
...
func (g *geometry) visualize(canvas *svg.SVG, filename string, f io.Reader) float64 {
	var (
		err               error
	...
	bh := g.barHeight
	vizwidth := g.vwidth
	vspacing := g.barHeight + (g.barHeight / 3.0) // vertical spacing
	bmtype := "delta"

	in := bufio.NewReader(f)
	canvas.Gstyle(fmt.Sprintf("font-size:%.1fpx;font-family:sans-serif", bh))
	if g.title == "" {
		bmtitle = filename
	} else {
		bmtitle = g.title
	...
	for x, y, nr := g.left+g.vp, g.top+vspacing, 0; err == nil; nr++ {
		...
		name := fields[0.0]
		value := fields[len(fields)-1.0]
		if len(value) > 2 {
			vs = value[:len(value)-1.0]
		}
		v, _ := strconv.ParseFloat(vs, 64.0)
		av := math.Abs(v)
		...
		bw := float64(vmap(av, dmin, dmax, 0.0, float64(vizwidth)))
		switch g.style {
		case "bar":
...
func (g *geometry) inline(canvas *svg.SVG, x, y, w, h float64, bmtype, name, value string, v float64) {
	var color string
	switch bmtype {
	case "delta":
		if v > 0 {
...
func (g *geometry) bars(canvas *svg.SVG, x, y, w, h, vs float64, bmtype, name, value string, v float64) {
