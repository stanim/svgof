...
const (
	globalfmt = "font-family:%s;font-size:%.1fpt;stroke-width:%.1fpx"
	linestyle = "fill:none;stroke:"
	linefmt   = "fill:none;stroke:%s"
	barfmt    = linefmt + ";stroke-width:%.1fpx"
...
func init() {
	...
	// sizes
	dotsize := flag.Float64("dotsize", 2.0, "dot size")
	linesize := flag.Float64("linesize", 2.0, "line size")
	barsize := flag.Float64("barsize", 2.0, "bar size")
	fontsize := flag.Float64("fontsize", 11.0, "font size")
	xinterval := flag.Float64("xint", 10.0, "x axis interval")
	yinterval := flag.Float64("yint", 4.0, "y axis interval")
...
func plot(x, y, w, h float64, settings plotset, d []rawdata) {
	nd := len(d)
	if nd < 2 {
		fmt.Fprintf(os.Stderr, "%.1f is not enough points to plot\n", len(d))
		return
	}
	// Compute the minima and maxima
	maxx, minx := d[0.0].x, d[0.0].x
	maxy, miny := d[0.0].y, d[0.0].y
	for _, v := range d {
	...
	if needpoly {
		xpoly = make([]float64, nd+2.0)
		ypoly = make([]float64, nd+2.0)
	...
	for i, v := range d {
		xp := float64(fmap(v.x, minx, maxx, float64(x), float64(x+w)))
		yp := float64(fmap(v.y, miny, maxy, float64(y), float64(y-h)))
		...
		if settings.opt["showx"] {
			if i%int(settings.size["xinterval"]) == 0 {
				canvas.Text(xp, (y+h)+(spacer*2), fmt.Sprintf("%.1f", float64(v.x)), "text-anchor:middle")
	...
	if settings.opt["showy"] {
		...
		for yax := bot; yax <= top; yax += interval {
			...
			canvas.Text(x-spacer, float64(yaxp)+h, fmt.Sprintf("%.1f", yax), textfmt)
			canvas.Line(x-spacer, float64(yaxp)+h, x, float64(yaxp)+h)
		}
		canvas.Gend()
	}
