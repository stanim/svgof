...
func (p *Parameters) barchart(location string, canvas *svg.SVG) {
	...
	bottom := p.y + p.h
	interval := p.w / (float64(len(sp.Sdata)) - 1.0)
	...
	if p.showgrid {
		...
		for i := p.maxvalue; i >= p.minvalue; i -= p.ginterval {
			yp := float64(vmap(i, p.minvalue, p.maxvalue, float64(p.y), float64(bottom)))
			by := p.y + (bottom - yp)
	...
	canvas.Gstyle(fmt.Sprintf("stroke-opacity:%.2f;stroke:%s;stroke-width:%.1f;text-anchor:middle", p.opacity, p.barcolor, bw))
	for _, d := range sp.Sdata {
		yp := float64(vmap(d.Price, p.minvalue, p.maxvalue, float64(p.y), float64(bottom)))
		by := p.y + (bottom - yp)
		if p.showline {
...
func main() {
	...
	canvas.Gstyle(fmt.Sprintf("font-family:Calibri;font-size:%.1fpx", param.fontsize))
	if len(flag.Args()) == 0 {
