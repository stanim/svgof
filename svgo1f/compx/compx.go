...
const (
	...
	globalstyle = "font-family:Calibri,sans-serif;font-size:%.1fpx;fill:black;text-anchor:middle;stroke-linecap:round;stroke-width:%.1fpx;stroke-opacity:%s"
	ltstyle     = "text-anchor:%s;fill:black"
	legendstyle = "text-anchor:start;fill:black;font-size:%.1fpx"
	gridstyle   = "fill:none; stroke:gray; stroke-opacity:0.3"
	notefmt     = "font-size:%.1fpx"
...
func grid(top, left, gutter float64) {
	gs := strings.SplitN(guide, `x`, 4.0)
	if len(gs) != 4 {
		return
	}
	w, _ := strconv.ParseFloat(gs[0.0], 64)
	h, _ := strconv.ParseFloat(gs[1.0], 64)
	nr, _ := strconv.ParseFloat(gs[2.0], 64)
	nc, _ := strconv.ParseFloat(gs[3.0], 64)
	...
	for r := 0; r < int(nr); r++ {
		...
		for c := 0; c < int(nc); c++ {
			canvas.Rect(x, y, w, h)
			canvas.Text(x+w/2, y+h/2, fmt.Sprintf("%.1f,%.1f", r, c),
...
func component(g gcomp, c comp) {
	...
	if len(c.Image) > 0 {
		...
		if len(c.Name) > 0 {
			canvas.Text(x+w2, y+h/3, c.Name,
				fmt.Sprintf("font-size:%.1fpx;fill:%s;baseline-shift:50%%", fs, boxcolor))
		}
		return
	}
	...
	case "mobile", "screen":
		...
		canvas.Gstyle(fmt.Sprintf("font-size:%.1fpx", fs))
	...
	case "server":
		...
		canvas.Text(x+w/2, y+h-10, c.Name, fmt.Sprintf("font-size:%.1fpx;fill:white", fs))
	...
	case "message":
		...
		canvas.Gstyle(fmt.Sprintf("font-size:%.1fpx;fill:white", fs))
		if len(c.Os) > 0 {
			canvas.Text(x+w2, y+pmy+fs, c.Os, "font-size:75%")
		}
		canvas.Text(x+w2, y+h-10, c.Name, fmt.Sprintf("font-size:%.1fpx;fill:%s", fs, bgcolor))
		canvas.Gend()
	...
	case "cloud":
		...
		canvas.Text(xc-(r/4), yc+r/2, c.Name, fmt.Sprintf("font-size:%.1fpx;fill:white", fs))

	case "db":
		cylinder(x, y+(h/4), w, h-(h/2), h/4, lcolor, boxcolor)
		canvas.Text(x+w2, y+h3, c.Name, fmt.Sprintf("font-size:%.1fpx;fill:white", fs))
		if len(c.Sw) > 0 {
	...
	case "face":
		fr := (w / 4.0) - (fs / 2.0) // (h*3)/8
		face(x+w/2, y+h/2, fr, linesize, boxcolor, bgcolor)
		canvas.Text(x+w/2, y+(h/2)+fr+15, c.Name, fmt.Sprintf("font-size:%.1fpx;fill:black", fs))

	case "role":
		role(x, y, w, h, boxcolor)
		canvas.Text(x+w/2, y+h-5, c.Name, fmt.Sprintf("font-size:%.1fpx;fill:%s", fs, bgcolor))
	...
	case "plain":
		...
		canvas.Text(x+w2, y+(h/2), c.Name, fmt.Sprintf("font-size:%.1fpx;fill:white;baseline-shift:-25%%", fs))
...
func uselibrary(x, y, w, h float64, name string) {
	canvas.Use(x, y, name, fmt.Sprintf(`width="%.1f"`, w), fmt.Sprintf(`height="%.1f"`, h))
}
...
func message(x, y, w, h, l float64, bcolor, scolor string) {
	...
	e1y := []float64{y + et, y + h, y + h, y + et, y + (et * 2.0), y + et}
	e2x := []float64{x, x + w2, x + w, x + w2, x}
	e2y := []float64{y + et, y, y + et, y + (et * 2.0), y + et}
...
func eaec(x, y, w, h, l float64, scolor, bcolor string) {
	...
	wh := w / 2.0
	hh := h / 2.0
	sx := []float64{x + wu*2.0, x + wu*6.0, x + wh}
	sy := []float64{y + hu*6.0, y + hu*6.0, y + hu*11.0}
	tx := []float64{x + wh, x + wh + wu, x + wh, x + wh - wu, x + wh}
	ty := []float64{y + hu*6.0, y + hu*7.0, y + hu*10.0, y + hu*7.0, y + hu*6.0}
...
func face(x, y, r, l float64, color, fcolor string) {
	...
	ep := 3.0 * fu
	my := y + ep
	canvas.Circle(x, y, r, fmt.Sprintf("fill:%s;stroke-width:%.1fpx;stroke:%s", fcolor, l, color))
	canvas.Circle(x+ep, y-ep, r/10, "fill:"+color)
	canvas.Circle(x-ep, y-ep, r/10, "fill:"+color)
	canvas.Qbez(x+ep, my, x, y+ep*2, x-ep, my, fmt.Sprintf("fill:%s;stroke-width:%.1fpx;stroke:%s", color, l, color))
}
...
func sloper(x1, y1, x2, y2 float64) (m, r float64) {
	...
	dx := float64(x1 - x2)
	m = dy / dx
	r = math.Atan2(dy, dx) * (180.0 / math.Pi)
	return m, r
}
...
func compass(g gcomp, point string) (cx, cy float64, dir string) {
	...
	case "nw":
		cx, cy, dir = g.x, g.y, "r"
	case "nnw":
		cx, cy, dir = g.x+g.w/4.0, g.y, "d"
	case "nne":
		cx, cy, dir = (g.x+g.w)-(g.w/4.0), g.y, "d"
	case "n":
		cx, cy, dir = g.x+(g.w/2.0), g.y, "d"
	case "ne":
		cx, cy, dir = g.x+g.w, g.y, "l"
	case "w":
		cx, cy, dir = g.x, g.y+g.h/2.0, "r"
	case "wnw":
		cx, cy, dir = g.x, g.y+g.h/4.0, "r"
	case "wsw":
		cx, cy, dir = g.x, (g.y+g.h)-(g.h/4.0), "r"
	case "ese":
		cx, cy, dir = g.x+g.w, (g.y+g.h)-(g.h/4.0), "l"
	case "ene":
		cx, cy, dir = g.x+g.w, g.y+(g.h/4.0), "l"
	case "c":
		cx, cy, dir = g.x+(g.w/2.0), g.y+(g.h/2.0), "n"
	case "e":
		cx, cy, dir = g.x+g.w, g.y+(g.h/2.0), "l"
	case "sw":
		cx, cy, dir = g.x, g.y+g.h, "r"
	case "ssw":
		cx, cy, dir = g.x+(g.w/4.0), g.y+g.h, "u"
	case "sse":
		cx, cy, dir = (g.x+g.w)-(g.w/4.0), g.y+g.h, "u"
	case "s":
		cx, cy, dir = g.x+(g.w/2.0), g.y+g.h, "u"
	case "se":
...
func doline(x1, y1, x2, y2 float64, style, direction, label string) {
	...
	hline := m == 0.0
	vline := m == math.Inf(-1.0) || m == math.Inf(1.0)
