...
var (
	width, height, iscale, fontsize, barheight, gutter, cornerRadius, labelimit float64
	bgcolor, barcolor, title, inbar, valformat                                  string
	showtitle, showdata, showgrid, showscale, endtitle, trace                   bool
)

const (
	gstyle      = "font-family:Calibri,sans-serif;font-size:%.1fpx"
...
func drawbc(bg Barchart, canvas *svg.SVG) {
	...
	for _, b := range bg.Bdata {
		...
		if len(sc) != 3 {
			...
			scalemin, _ = strconv.ParseFloat(sc[0.0], 64.0)
			scalemax, _ = strconv.ParseFloat(sc[1.0], 64.0)
			scaleincr, _ = strconv.ParseFloat(sc[2.0], 64.0)
		}
		// label the graph
		...
		for _, stack := range b.Bstack {
			...
			sx := x
			canvas.Text(x-sep, y+barheight/2, textlimit(stack.Name, int(labelimit)), labelstyle)
			barop := colorange(1.0, 0.3, len(stackdata))
			for ns, sd := range stackdata {
				dw := vmap(sd, scalemin, scalemax, 0.0, float64(maxwidth))
				if len(stack.Color) > 0 {
					canvas.Roundrect(sx, y, float64(dw), barheight, cr, cr, fmt.Sprintf("fill:%s;fill-opacity:%.2f", stack.Color, barop[ns]))
				} else {
					canvas.Roundrect(sx, y, float64(dw), barheight, cr, cr, fmt.Sprintf("fill-opacity:%.2f", barop[ns]))
				}
		...
		for _, d := range b.Bitem {
			...
			canvas.Text(x-sep, y+barheight/2, textlimit(d.Name, int(labelimit)), labelstyle)
			dw := vmap(d.Value, scalemin, scalemax, 0.0, float64(maxwidth))
			var barop float64
			if b.Samebar {
				barop = 0.3
			} else {
			...
			if len(d.Color) > 0 {
				canvas.Roundrect(x, y, float64(dw), barheight, cr, cr, fmt.Sprintf("fill:%s;fill-opacity:%.2f", d.Color, barop))
			} else {
				canvas.Roundrect(x, y, float64(dw), barheight, cr, cr, fmt.Sprintf("fill-opacity:%.2f", barop))
			}
			if showdata || b.Showdata {
				...
				if len(inbar) > 0 {
					valuestyle += inbar
					datax = x + fontsize/2.0
				} else {
					valuestyle += "fill:black"
					datax = x + float64(dw) + fontsize/2.0
				}
				canvas.Text(datax, y+barheight/2, ditem, valuestyle)
			}
			if !d.Samebar {
		...
		if showscale {
			...
			for sc := scalemin; sc <= scalemax; sc += scaleincr {
				scx := vmap(sc, scalemin, scalemax, 0.0, float64(maxwidth))
				canvas.Text(x+float64(scx), chartbot+fontsize, fmt.Sprintf(scfmt, sc))
				if showgrid || b.Showgrid {
					canvas.Line(x+float64(scx), chartbot, x+float64(scx), chartop, borderstyle) // grid line
				}
			}
			canvas.Gend()
		}
		...
		y += sep * 7.0 // advance vertically for the next chart
	}
...
func scalevalues(data []bitem) (float64, float64, float64) {
	...
	max = roundup(m, 100.0)
	if max > 2 {
		increment = roundup(max/float64(rui), 10.0)
	} else {
		increment = 0.4
	}
...
func stackvalues(s string) []float64 {
	...
	for i, x := range v {
		f, err := strconv.ParseFloat(x, 64.0)
		if err != nil {
			vals[i] = 0.0
		} else {
			vals[i] = f
