...
var (
	...
	gstyle                                                   = "font-family:'%s',sans-serif;font-size:%.1fpx"
)
...
func drawbg(bg Bulletgraph, canvas *svg.SVG) {
	...
	for _, v := range bg.Bdata {
		...
		for i, q := range qm {
			qmeasures[i], _ = strconv.ParseFloat(q, 64.0)
		}
		...
		for sc := scalemin; sc <= scalemax; sc += scaleincr {
			...
			canvas.Text(x+float64(scx), y+scalesep+barheight+fontsize/2, fmt.Sprintf(scfmt, sc))
		}
		canvas.Gend()
		...
		for _, q := range qmeasures {
			qbarlength := vmap(q, scalemin, scalemax, 0.0, float64(maxwidth))
			canvas.Rect(x, y, float64(qbarlength), barheight)
		}
		canvas.Gend()

		// draw the measure and the comparative measure
		barlength := float64(vmap(v.Measure, scalemin, scalemax, 0.0, float64(maxwidth)))
		canvas.Rect(x, y+qmheight, barlength, qmheight, "fill:"+datacolor)
		cmx := float64(vmap(v.Cmeasure, scalemin, scalemax, 0.0, float64(maxwidth)))
		if circlemark {
