...
var (
	...
	opacity                                 float64
	showtitle                               bool
	gstyle                                  = "font-family:Calibri;font-size:%.1fpx;text-anchor:middle"
...
func drawrr(rm Roadmap) {
	...
	for i := 0; i < int(duration); i++ {
		...
		canvas.Text(cx, (cy - sr), fmt.Sprintf("%s %.1f", rm.Unit, i+1), "font-size:150%;fill:gray")
		sr += r
	}
	...
	for _, s := range rm.Section {
		...
		spacing := s.Spacing
		if spacing == 0 {
			spacing = (angle / float64(len(s.Item))) - 1.0
		}
		iangle := a + spacing
...
func drawseclines(cx, cy float64, size, a, h float64, s string) {
	fs := fontsize + (fontsize / 2.0)
	ix, iy := dpolar(cx, cy, size, a)
	ix2, iy2 := dpolar(cx, cy, size+50.0, a+h)
...
func textlines(x, y, fs, leading float64, align, fill string, s []string) {
	canvas.Gstyle(fmt.Sprintf("font-size:%.1fpx;text-anchor:%s;fill:%s", fs, align, fill))
	for _, v := range s {
		canvas.Text(x, y, v)
		y += leading
	}
...
func init() {
	...
	flag.Float64Var(&iscale, "s", float64(float64(width)*.009), "impact scale")
