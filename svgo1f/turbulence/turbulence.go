// The original library github.com/ajstarks/svgo uses int coordinates.
// This port "svgo1f" uses float coordinates, which are formatted as "%.1f". 
// This file was automatically generated by modifying the syntax tree.
// (There are other precisions available at github.com/stanim/svgo.)
//
// turbulence example from http://www.w3.org/TR/2003/REC-SVG11-20030114/filters.html#feTurbulence
// +build !appengine

package main

import (
	"fmt"
	"github.com/stanim/svgo/svgo1f"
	"os"
)

var (
	canvas = svg.New(os.Stdout)
	width  = 500.0
	height = 500.0
)

type perlin struct {
	id        string
	ftype     string
	basefreqx float64
	basefreqy float64
	octave    float64
	seed      int64
	tile      bool
}

func (p perlin) defturbulence() {
	x := svg.Filterspec{}
	canvas.Filter(p.id)
	canvas.FeTurbulence(x, p.ftype, p.basefreqx, p.basefreqy, p.octave, p.seed, p.tile)
	canvas.Fend()
}

func (p perlin) frect(x, y, w, h float64) {
	bot := y + h
	canvas.Rect(x, y, w, h, fmt.Sprintf(`filter="url(#%s)"`, p.id))
	canvas.Text(x+w/2, bot+25, fmt.Sprintf("type=%s", p.ftype))
	canvas.Text(x+w/2, bot+40, fmt.Sprintf("baseFrequency=%.2f", p.basefreqx))
	canvas.Text(x+w/2, bot+55, fmt.Sprintf("numOctaves=%.1f", p.octave))
}

func main() {
	var t1, t2, t3, t4, t5, t6 perlin

	t1 = perlin{"Turb1", "t", 0.05, 0.05, 2.0, 0.0, false}
	t2 = perlin{"Turb2", "t", 0.10, 0.10, 2.0, 0.0, false}
	t3 = perlin{"Turb3", "t", 0.05, 0.05, 8.0, 0.0, false}
	t4 = perlin{"Turb4", "f", 0.10, 0.10, 4.0, 0.0, false}
	t5 = perlin{"Turb5", "f", 0.40, 0.40, 4.0, 0.0, false}
	t6 = perlin{"Turb6", "f", 0.10, 0.10, 1.0, 0.0, false}

	canvas.Start(width, height)
	canvas.Title("Example of feTurbulence")
	canvas.Def()
	t1.defturbulence()
	t2.defturbulence()
	t3.defturbulence()
	t4.defturbulence()
	t5.defturbulence()
	t6.defturbulence()
	canvas.DefEnd()

	canvas.Gstyle("font-size:10;font-family:Verdana;text-anchor:middle")
	t1.frect(25, 25, 100, 75)
	t2.frect(175, 25, 100, 75)
	t3.frect(325, 25, 100, 75)
	t4.frect(25, 180, 100, 75)
	t5.frect(175, 180, 100, 75)
	t6.frect(325, 180, 100, 75)
	canvas.Gend()
	canvas.End()
}
