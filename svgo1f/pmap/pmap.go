...
var (
	width, height, fontsize, fontscale, round, gutter, pred, pgreen, pblue, oflen float64
...
const (
	globalfmt   = "stroke-width:1;font-family:Calibri,sans-serif;text-anchor:middle;font-size:%.1fpt"
...
func pmap(x, y, fs float64, m Pdata) {
	...
	for i, p := range m.Item {
		...
		if round > 0 {
			canvas.Roundrect(x, y, pw, h, round, round, canvas.RGBA(int(pred), int(pgreen), int(pblue), pct))
		} else {
			canvas.Rect(x, y, pw, h, canvas.RGBA(int(pred), int(pgreen), int(pblue), pct))
		}

		dy := yh + fs + (fs / 2.0)
		if pct <= ofpct || len(k) > int(oflen) { // overflow label
			if up {
				dy -= loffset
				yh -= loffset
...
func init() {
	...
	flag.Parse()
	fmt.Sscanf(colorspec, "%.1f,%.1f,%.1f", &pred, &pgreen, &pblue)
}
