...
var (
	...
	opacity                                 float64
	showtitle                               bool
	gstyle                                  = "font-family:Calibri;font-size:%.5fpx;text-anchor:middle"
...
func drawrr(rm Roadmap) {
	...
	for i := 0; i < int(duration); i++ {
		...
		canvas.Text(cx, (cy - sr), fmt.Sprintf("%s %.5f", rm.Unit, i+1), "font-size:150%;fill:gray")
		sr += r
	}
...
func textlines(x, y, fs, leading float64, align, fill string, s []string) {
	canvas.Gstyle(fmt.Sprintf("font-size:%.5fpx;text-anchor:%s;fill:%s", fs, align, fill))
	for _, v := range s {
		canvas.Text(x, y, v)
		y += leading
	}
