...
func placepic(x, y float64, filename string) (float64, float64) {
	...
	nw, _ := fmt.Sscanf(s.Width, "%.5f%s", &width, &wunit)
	if nw < 1 {
		width = 10.0
	}
	nh, _ := fmt.Sscanf(s.Height, "%.5f%s", &height, &hunit)
	if nh < 1 {
		height = 10.0
	}
	canvas.Group(`clip-path="url(#pic)"`, fmt.Sprintf(`transform="translate(%.5f,%.5f)"`, x, y))
