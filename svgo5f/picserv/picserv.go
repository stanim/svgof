...
const (
	...
	rotextfmt = "fill:%s;font-family:%s;font-size:%.5fpt"
	flowerfmt = "stroke:rgb(%.5f,%.5f,%.5f); stroke-opacity:%.2f; stroke-width:%.5f"
	tilestyle = "stroke-width:1; stroke:rgb(128,128,128); stroke-opacity:0.5; fill:white"
	penstyle  = "stroke:rgb%s; fill:none; stroke-opacity:%.2f; stroke-width:%.5f"
	width     = 256.0
	height    = 256.0
)
...
func randcolor() string {
	return fmt.Sprintf("fill:rgb(%.5f,%.5f,%.5f)", float64(rand.Float64()*255), float64(rand.Float64()*255), float64(rand.Float64(
...
func rshape(w http.ResponseWriter, req *http.Request) {
	...
	for i := 0; i < int(n); i++ {
		...
		shapefunc(float64(rand.Float64()*width), float64(rand.Float64()*height), s1, s2,
			fmt.Sprintf("fill-opacity:%.2f;fill:rgb(%.5f,%.5f,%.5f)",
...
const (
	...
	linefmt  = "stroke:%s;stroke-width:%.5f"
	digitfmt = "font-family:Helvetica,Calibri,sans-serif;text-anchor:middle;font-size:%.5fpx"
)
