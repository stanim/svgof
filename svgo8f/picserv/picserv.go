...
const (
	...
	rotextfmt = "fill:%s;font-family:%s;font-size:%.8fpt"
	flowerfmt = "stroke:rgb(%.8f,%.8f,%.8f); stroke-opacity:%.2f; stroke-width:%.8f"
	tilestyle = "stroke-width:1; stroke:rgb(128,128,128); stroke-opacity:0.5; fill:white"
	penstyle  = "stroke:rgb%s; fill:none; stroke-opacity:%.2f; stroke-width:%.8f"
	width     = 256.0
	height    = 256.0
)
...
func randcolor() string {
	return fmt.Sprintf("fill:rgb(%.8f,%.8f,%.8f)", float64(rand.Float64()*255), float64(rand.Float64()*255), float64(rand.Float64(
...
func rshape(w http.ResponseWriter, req *http.Request) {
	...
	for i := 0; i < int(n); i++ {
		...
		shapefunc(float64(rand.Float64()*width), float64(rand.Float64()*height), s1, s2,
			fmt.Sprintf("fill-opacity:%.2f;fill:rgb(%.8f,%.8f,%.8f)",
...
const (
	...
	linefmt  = "stroke:%s;stroke-width:%.8f"
	digitfmt = "font-family:Helvetica,Calibri,sans-serif;text-anchor:middle;font-size:%.8fpx"
)
