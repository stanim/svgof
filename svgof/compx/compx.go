...
const (
	...
	globalstyle = "font-family:Calibri,sans-serif;font-size:%fpx;fill:black;text-anchor:middle;stroke-linecap:round;stroke-width:%fpx;stroke-opacity:%s"
	ltstyle     = "text-anchor:%s;fill:black"
	legendstyle = "text-anchor:start;fill:black;font-size:%fpx"
	gridstyle   = "fill:none; stroke:gray; stroke-opacity:0.3"
	notefmt     = "font-size:%fpx"
...
func grid(top, left, gutter float64) {
	...
	for r := 0; r < int(nr); r++ {
		...
		for c := 0; c < int(nc); c++ {
			canvas.Rect(x, y, w, h)
			canvas.Text(x+w/2, y+h/2, fmt.Sprintf("%f,%f", r, c),
...
func component(g gcomp, c comp) {
	...
	if len(c.Image) > 0 {
		...
		if len(c.Name) > 0 {
			canvas.Text(x+w2, y+h/3, c.Name,
				fmt.Sprintf("font-size:%fpx;fill:%s;baseline-shift:50%%", fs, boxcolor))
		}
		return
	}
	...
	case "mobile", "screen":
		...
		canvas.Gstyle(fmt.Sprintf("font-size:%fpx", fs))
	...
	case "server":
		...
		canvas.Text(x+w/2, y+h-10, c.Name, fmt.Sprintf("font-size:%fpx;fill:white", fs))
	...
	case "message":
		...
		canvas.Gstyle(fmt.Sprintf("font-size:%fpx;fill:white", fs))
		if len(c.Os) > 0 {
			canvas.Text(x+w2, y+pmy+fs, c.Os, "font-size:75%")
		}
		canvas.Text(x+w2, y+h-10, c.Name, fmt.Sprintf("font-size:%fpx;fill:%s", fs, bgcolor))
		canvas.Gend()
	...
	case "cloud":
		...
		canvas.Text(xc-(r/4), yc+r/2, c.Name, fmt.Sprintf("font-size:%fpx;fill:white", fs))

	case "db":
		cylinder(x, y+(h/4), w, h-(h/2), h/4, lcolor, boxcolor)
		canvas.Text(x+w2, y+h3, c.Name, fmt.Sprintf("font-size:%fpx;fill:white", fs))
		if len(c.Sw) > 0 {
	...
	case "face":
		...
		canvas.Text(x+w/2, y+(h/2)+fr+15, c.Name, fmt.Sprintf("font-size:%fpx;fill:black", fs))

	case "role":
		role(x, y, w, h, boxcolor)
		canvas.Text(x+w/2, y+h-5, c.Name, fmt.Sprintf("font-size:%fpx;fill:%s", fs, bgcolor))
	...
	case "plain":
		...
		canvas.Text(x+w2, y+(h/2), c.Name, fmt.Sprintf("font-size:%fpx;fill:white;baseline-shift:-25%%", fs))
...
func uselibrary(x, y, w, h float64, name string) {
	canvas.Use(x, y, name, fmt.Sprintf(`width="%f"`, w), fmt.Sprintf(`height="%f"`, h))
}
...
func face(x, y, r, l float64, color, fcolor string) {
	...
	ep := 3.0 * fu
	my := y + ep
	canvas.Circle(x, y, r, fmt.Sprintf("fill:%s;stroke-width:%fpx;stroke:%s", fcolor, l, color))
	canvas.Circle(x+ep, y-ep, r/10, "fill:"+color)
	canvas.Circle(x-ep, y-ep, r/10, "fill:"+color)
	canvas.Qbez(x+ep, my, x, y+ep*2, x-ep, my, fmt.Sprintf("fill:%s;stroke-width:%fpx;stroke:%s", color, l, color))
}
