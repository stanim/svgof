...
const (
	globalfmt = "font-family:%s;font-size:%.2fpt;stroke-width:%.2fpx"
	linestyle = "fill:none;stroke:"
	linefmt   = "fill:none;stroke:%s"
	barfmt    = linefmt + ";stroke-width:%.2fpx"
...
func plot(x, y, w, h float64, settings plotset, d []rawdata) {
	nd := len(d)
	if nd < 2 {
		fmt.Fprintf(os.Stderr, "%.2f is not enough points to plot\n", len(d))
		return
	}
	...
	for i, v := range d {
		...
		if settings.opt["showx"] {
			if i%int(settings.size["xinterval"]) == 0 {
				canvas.Text(xp, (y+h)+(spacer*2), fmt.Sprintf("%.2f", float64(v.x)), "text-anchor:middle")
