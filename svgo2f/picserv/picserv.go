...
const (
	...
	rotextfmt = "fill:%s;font-family:%s;font-size:%.2fpt"
	flowerfmt = "stroke:rgb(%.2f,%.2f,%.2f); stroke-opacity:%.2f; stroke-width:%.2f"
...
func randcolor() string {
	return fmt.Sprintf("fill:rgb(%.2f,%.2f,%.2f)", float64(rand.Float64()*255), float64(rand.Float64()*255), float64(rand.Float64(
...
func rshape(w http.ResponseWriter, req *http.Request) {
	...
	for i := 0; i < int(n); i++ {
		...
		shapefunc(float64(rand.Float64()*width), float64(rand.Float64()*height), s1, s2,
			fmt.Sprintf("fill-opacity:%.2f;fill:rgb(%.2f,%.2f,%.2f)",
...
const (
	...
	digitfmt = "font-family:Helvetica,Calibri,sans-serif;text-anchor:middle;font-size:%.2fpx"
)
